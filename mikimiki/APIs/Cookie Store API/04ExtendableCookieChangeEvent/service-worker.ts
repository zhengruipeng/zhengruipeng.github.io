...
type ServiceWorkerGlobalScope = Window;
...
class ServiceWorkerHandler extends Object {
    public self: ServiceWorkerGlobalScope;
    ...
    install(): void {
        ...
        // @ts-ignore
        let registration: ServiceWorkerRegistration = this.self.registration;
        // @ts-ignore
        let manager: CookieStoreManager = registration.cookies;
    ...
    message(ev): void {
        ...
        let mapperObserveKey: MapperObserveKey = new MapperObserveKey();
        mapperObserveKey.addKey("type");

        this.messageMapper = new Mapper(ev.data, {observeKey: mapperObserveKey});

        this.messageMapper.map("subscribe", async function (): Promise<undefined> {
            ...
            let params: Array<Subscriptions> = [{name, url}];
            return await manager.subscribe(params);
        });

        this.messageMapper.map("unsubscribe", async function (): Promise<undefined> {
        ...
        this.messageMapper.map("getSubscriptions", async function (): Promise<Array<Subscriptions>> {
            let subscriptions = await manager.getSubscriptions();

            let msg: string = "";
            subscriptions.forEach((v: Subscriptions) => {
    ...
    cookieChange(ev): void {
        console.log("change event invoked:",
            ev);
    }
}

let handler: ServiceWorkerHandler = new ServiceWorkerHandler(self);
