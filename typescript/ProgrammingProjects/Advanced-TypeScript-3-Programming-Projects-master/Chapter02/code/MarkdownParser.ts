...
//通过枚举类型的标签类型，建立一个类型到html标签文本的映射。
//之后用tm两个蜜汁函数生成了html标签。
class TagTypeToHtml {
...
//维护一个content属性，整一个add方法，向content中添加内容
//get方法就是取出content;
...
//通过行的方式检查输入文本，维护一个output数组，
// 如果文本以特定的字符串开头的时候，
// 把output中的第一个布尔值改成true并且删除掉特定字符串
//说白了就是# aaa开头的时候只显示aaa
class LineParser {
