...
export default {
    ...
    actions: {
        ...
        async getListForPage({commit}: { commit: any }, data: any): Promise<any> {
            // 调用用户的分页查询接口
            let res = await GoodsApi.listPage(data);
        ...
        async deleteById({dispatch}, id: string): Promise<any> {
            return await GoodsApi.deleteId(id)
        },

        async insert(_: any, data: Object): Promise<any> {
            return await GoodsApi.insert(data)
        },

        async update({}, addForm: Object): Promise<any> {
            return await GoodsApi.update(addForm)
        },

        async findId({}: any, id: string) {
            let res = await GoodsApi.findId(id)
            if (res.data.code === 200) {
        ...
        async onSale({}: any, id: string, isOnSale: boolean) {
            let res = await GoodsApi.onSale(id, isOnSale);
            if (res.data.code === 200) {
                return res.data.data
            } else {
                return {}
