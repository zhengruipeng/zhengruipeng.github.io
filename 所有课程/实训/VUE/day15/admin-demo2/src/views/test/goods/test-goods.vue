<template>
  <div class="page">
    ...
    <el-form inline :model="queryForm">
      ...
      <el-form-item size="mini">
        <el-button icon="el-icon-plus" type="success" @click="handleAdd">增加</el-button>
      </el-form-item>
    </el-form>
    <!--
      使用table组件
    ...
    <el-table
      ...
      <el-table-column label="折扣" prop="zheKou">
        <!--
          这个是el-table-column
          ...
          <el-tag size="small">
            {{
              new Intl.NumberFormat(
                  ...
                  .format(Number.parseFloat(row.zheKou)) + "折"
            }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="是否贩卖" prop="isOnSale">
        <template v-slot="{row}">

          <el-button @click="switchSaleStatus(row.id, row.isOnSale)">
            改为{{ row.isOnSale === "1" ? "下架" : "上架" }}
          </el-button>
        </template>
      </el-table-column>
...
<!--          <el-popover
              ...
              trigger="hover"
              :content="row.remark">
            <el-button>{{ textTruncate(row.remark, 100) }}</el-button>
          </el-popover>-->
          <el-popover
              placement="top-start"
              title="标题"
              ...
              trigger="hover"
              :content="row.remark">
            <div slot="reference">{{ textTruncate(row.remark, 100) }}</div>
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <!--
         ...
         -->
        <template v-slot="{row}">
          <el-button type="warning" size="mini" icon="el-icon-edit" @click="handleEdit(row.id)">修改</el-button>
          <el-button type="danger" size="mini" icon="el-icon-remove" @click="handleRemove(row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  async created() {
    let res = await this.getListForPage(this.queryForm)
  },
  ...
  methods: {
    ...
    ...mapActions('goodsModel', ['getListForPage', 'deleteById', "onSale"]),
    // 点击查询按钮触发的⌚️
    async handleClick() {
    ...
    async switchSaleStatus(id, isOnSale) {
      this.$confirm('确定转换？', '标题', {
        ...
        this.onSale(id, isOnSale !== "1").then(_ => {
          let res = this.getListForPage(this.queryForm)
        })
      }).catch(err => {
        //点击取消时的回调，err为cancel
      });
